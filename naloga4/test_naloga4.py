...
def test_naloga4(case_dir, case_id):
    ...
    start = timer()
    izhod = naloga4(data['vhod'], data['fs'])
    end = timer()
    t_elapsed = end - start
    ...
    if success != 1:
        print(' ✗ Napačen `izhod`')
        print(f'   -> Najdeni akord: "{izhod}"')
        print(f'   -> Pravilen akord: "{data["izhod"]}"')
...
if __name__ == "__main__":
    ...
    for i in range(10):
        test_naloga4('old_json', i+1)
    # test_naloga4('primeri', 1)
    # test_naloga4('primeri', 2)
    # test_naloga4('primeri', 3)
